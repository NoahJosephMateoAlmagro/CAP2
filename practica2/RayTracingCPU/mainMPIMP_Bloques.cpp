...
void rayTracingCPU_MPIMP(unsigned char *img, int w, int h, int ns = 10, int px = 0, int py = 0, int pw = -1, int ph = -1)
{
	if (pw == -1)
		pw = w;
	if (ph == -1)
...
int main(int argc, char *argv[])
{
	...
	rayTracingCPU_MPIMP(data, w, h, ns);
	...
	if (rank == 0)
	{
		writeBMP("imgMPIMPImgBlocks.bmp", data, w, h);
	...
	if (rank == 0)
	{
		printf("Tiempo de ejecución con MPI y OMP: %f segundos.\n", end_time - start_time);
	}
