...
void rayTracingCPU_OMP_Fil(unsigned char *img, int w, int h, int ns = 10, int px = 0, int py = 0, int pw = -1, int ph = -1)
{
    if (pw == -1) pw = w;
    if (ph == -1) ph = h;
...
int main(int argc, char *argv[])
{
    ...
    rayTracingCPU_OMP_Fil(data, w, h, ns);

    double end_time = omp_get_wtime();

    writeBMP("imgOMPFilas.bmp", data, w, h);
