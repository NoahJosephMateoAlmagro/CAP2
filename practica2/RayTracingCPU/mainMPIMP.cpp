...
void rayTracingCPU_MPI_MP(unsigned char *img, int w, int h, int ns = 10, int px = 0, int py = 0, int pw = -1, int ph = -1)
{
    if (pw == -1)
        pw = w;
    if (ph == -1)
...
// Renderizar solo la parte correspondiente para cada proceso
#pragma omp parallel for
	for (int j = filaInicial; j < filaFinal; j++)
	{
		for (int i = 0; i < w; i++)
		{
			...
			local_img[(j - filaInicial) * w * 3 + i * 3 + 2] = char(255.99 * col[0]);
			local_img[(j - filaInicial) * w * 3 + i * 3 + 1] = char(255.99 * col[1]);
			local_img[(j - filaInicial) * w * 3 + i * 3 + 0] = char(255.99 * col[2]);
		}
	}
...
int main(int argc, char *argv[])
{
	...
	int w = 2048; // Ancho de la imagen
	int h = 2048; // Alto de la imagen
	...
	rayTracingCPU_MPI_MP(data, w, h, ns);
	...
	if (rank == 0)
	{
		writeBMP("imgMPIImg.bmp", data, w, h);
	...
	if (rank == 0)
	{
		printf("Tiempo de ejecución con MPI + OPENMP: %f segundos.\n", end_time - start_time);
	}
